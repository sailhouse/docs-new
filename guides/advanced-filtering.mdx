...
You can also configure advanced filters in your schema files:

```yaml
subscriptions:
  - slug: enterprise-alerts
    topic: system-events
    type: push
    endpoint: https://alerts.enterprise.com/webhook
    filter:
      filters:
        - path: severity
          condition: equals
  ...
  - slug: user-notifications
    topic: user-events
    type: push
    endpoint: https://notifications.app.com/webhook
    filter:
      filters:
